...
export default function CreateListing() {
  ...
  async function onSubmit(e){
    e.preventDefault();
    setLoading(true)
    if(+descountedPrice >= +regularPrice){
      setLoading(false);
      toast.error("Regular price is less than discount")
      return;
    }
    if(images.length > 6){
      setLoading(false)
    ...
    if(geolocationEnabled){
        toast.error("please enter latitude and logitude in address")
    }
    else{
  ...
  () => {
    ...
    !formDataCopy.offer && delete formDataCopy.descountedPrice;
      ...
      <form onSubmit={onSubmit}>
        ...
        {!geolocationEnabled && (
          <div className="flex space-x-6 justify-start mb-6 ">
            <div className="">
              ...
              <input type='number' id='latitude' className="w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:bg-white focus:text-gray-700 focus:border-slate-600
              text-center" value={latitude} onChange={onChange} required min="-90" max="90"/>
            </div>
            <div className="">
              <p className="text-lg font-semibold">Longitude</p>
              <input type='number' id='longitude' className="w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:bg-white focus:text-gray-700 focus:border-slate-600
              text-center" value={longitude} onChange={onChange} required min="-180" max="180"/>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="flex w-full justify-center items-center space-x-6">
            ...
            {type === "rent" && (
              <div>
                <p className="text-md w-full text-xl whitespace-nowrap">
                  $/month
                </p>
              </div>
            )}
        ...
        {offer && (
          <div>
            <p className="text-lg font-semibold mt-6">Descounted price</p>
            <div className="flex w-full justify-center items-center space-x-6">
              <input
                type="number"
                id="descountedPrice"
                value={descountedPrice}
                onChange={onChange}
                min="50"
                max="40000000"
                required={offer}
        ...
        uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Create Listing</button>
      </form>
    </main>
  );
}
